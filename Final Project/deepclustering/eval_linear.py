...
parser.add_argument('--labeled', type=int, default=64, help='Number of labeled examples per class')
...
def main():
    ...
    train_indices = [item for sublist in [list(map(lambda x: x + 64 * i, random.sample(range(64), args.labeled))) for i in range(1000)] for item in sublist]
    ...
    print(f'Using {len(train_indices)} labeled examples in the training set ({args.labeled} per class)')
    ...
    for epoch in range(args.epochs):
        ...
        if is_best:
            filename = f'model_best_{args.labeled}.pth.tar'
        else:
            filename = f'checkpoint_{args.labeled}.pth.tar'
        torch.save({
            'epoch': epoch + 1,
            'arch': 'alexnet',
